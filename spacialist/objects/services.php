...
class Service{
    ...
    private $table_name1= "bus_service_price";
    private $table_name2= "business";
    private $table_name3 = "service";
    ...
    public $selected_date= "dd/mmm/yyyy";
    ...
    $query = "SELECT b.bus_id,b.bus_name,s.service_id,s.service_name,s.service_desc,s.duration,s.service_type,bsp.price from business b inner join bus_service_price bsp on b.bus_id=bsp.bus_id inner join service s on bsp.service_id=s.service_id where b.bus_id=? ORDER BY s.service_type ASC ";
    ...
    return $stmt;
     ...
     function readAllServicesByStaff($selected_time, $selected_date){
        $query="SELECT bus.bus_id, bus.bus_name, ss.staff_service_id, s.service_id, s.service_name, s.service_desc, s.duration, s.service_type,bsp.price, staff_service_avail.staff_id, st.staff_logo,st.firstname 
            from business bus
                        ...
                        SELECT sub_ss.staff_id from staff_service sub_ss
                            WHERE 
                                ...
                                WHERE 
                                    ...
                                    WHERE   
                                    (
                                        (TIME(\"$selected_time\") < sd.start_time AND TIME(\"$selected_time\")+s.duration < sd.end_time AND  TIME(\"$selected_time\")+s.duration > sd.start_time)
                                        OR (TIME(\"$selected_time\") >= sd.start_time AND TIME(\"$selected_time\")+s.duration <= sd.end_time)
                                        OR (TIME(\"$selected_time\") <= sd.start_time AND TIME(\"$selected_time\")+s.duration >= sd.end_time)
                                        OR (TIME(\"$selected_time\") > sd.start_time AND TIME(\"$selected_time\")+s.duration > sd.end_time AND TIME(\"$selected_time\") < sd.end_time)
                                        )
                                    AND sd.sched_date = date(\"$selected_date\")
                                    /*TIME(\"$selected_time\") >= sd.start_time
            ...
            ORDER BY staff_service_avail.staff_id,s.service_type ASC";
